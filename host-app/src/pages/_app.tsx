...
function MyApp({ Component, pageProps }: AppProps) {
  ...
  useEffect(() => {
    const loadSidebarItems = async () => {
      ...
      } else if (router.pathname.startsWith("/remote-react-app")) {
        try {
          const { sidebarItems } = await import("reactApp/sidebarItems");
          console.log("sidebarItems", sidebarItems);
  ...
  const handleSelect = (item: SidebarItem) => {
    ...
    router.pathname.startsWith("/remote-app")
      ? router.push(`/remote-app/${item.path}`)
      : router.push(`/remote-react-app/${item.path}`);
  };
